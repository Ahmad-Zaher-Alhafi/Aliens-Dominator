...
public class FlyingSystem : MonoBehaviour
{
    ...
    [SerializeField] Vector3 patrolUnWantedPositions;//positions between (-unWantedPositions and unWantedPositions) will add to them the patrolOffset
    [SerializeField] Vector3 patrolOffset;//to prevent close position to the target when the air creature fly near ro it
    [SerializeField] private List<StinkyBall> stinkyBallPrefabs = new List<StinkyBall>();
    ...
    private List<Transform> airWayPoints = new List<Transform>();//air points which the creature has to follow
    ...
    private List<Paths> creaturePathes = new List<Paths>();
    public List<Paths> CreaturePathes
    {
        set
        {
            creaturePathes = value;
        }
    }
    ...
    void Start()
    {
        ...
        {
            EventsManager.onStartEnemyDeath += OrderToRunAway;
        }
        ...
        {
            airWayPoints.Add(pathWaypoints[i].transform);//get the transorms of these way points
        }
    ...
    public void FollowTarget(Transform target)
    {
        ...
        if (hasToPatrol)//if it has to patrol around
        {
            if (isNotTargetingPlayer == 1)
            {
                StartPatrolling(target);
            }
            else
            {
                StartPatrolling(pointToAttackThePlayer);
            }
        }
        else
        {
            if (!IsItGroupMember || IsItTheLeader)
            {
                transform.position = Vector3.Lerp(transform.position, target.position, creature.EnemySpeed * Time.deltaTime / Vector3.Distance(transform.position, target.position));//make the creature moves
            }
        }
    }
    ...
    private void PatrolAround(Transform target)//this function is gonna let the player move from random position to another
    {
        ...
        {
            if (Mathf.Abs(Vector3.Distance(transform.position, target.position)) > .2f && !hasFinishedAttacking)
            {
                transform.position = Vector3.Lerp(transform.position, target.position, patrollingSpeed * Time.deltaTime / Vector3.Distance(transform.position, target.position));//move the creautre to the player position
            }
    ...
    private void GetNewPatrolPosition(Transform pointToPatrolAround)//function to generate random positions around that point
    {
        ...
        {
            if (randomPatrolPosiotion.x < 0)
            {
                randomPatrolPosiotion.x -= patrolOffset.x;
            }
            else
            {
                randomPatrolPosiotion.x += patrolOffset.x;
        ...
        {
            if (randomPatrolPosiotion.y < 0)
            {
                randomPatrolPosiotion.y -= patrolOffset.y;
            }
            else
            {
                randomPatrolPosiotion.y += patrolOffset.y;
        ...
        if (Mathf.Abs(randomPatrolPosiotion.z - pointToPatrolAround.position.z) < patrolUnWantedPositions.z && Mathf.Abs(randomPatrolPosiotion.z - pointToPatrolAround.position.z) > -patrolUnWantedPositions.z)
        {
            randomPatrolPosiotion.z -= patrolOffset.z;
        }
    }
    ...
    public IEnumerator ThrewStinkyBall(Transform target)
    {
        ...
        foreach (StinkyBall sB in stinkyBallPrefabs)
        {
            if (sB.StinkyBallColor == creatureColor)//find the stinky ball that matchs the creature color
            {
                ...
                if (stinkyBall != null)
                {
                    stinkyBall.transform.parent = BodyRig.transform;//make the stinky ball parent is the BodyRig of the creature who created it
                }

                Animator stinkyAnimator = GetComponent<Animator>();
                ...
                if (stinkyBall != null)
                {
                    ...
                    if (stinkyBall != null)
                    {
                        if (isNotTargetingPlayer == 1)
                        {
                            stinkyBall.GetComponent<StinkyBall>().FollowTarget(target);
                        }
                        else
                        {
                            stinkyBall.GetComponent<StinkyBall>().FollowTarget(pointToShootAtThePlayer);
                        }
                    }
                }

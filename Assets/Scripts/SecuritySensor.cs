...
public class SecuritySensor : MonoBehaviour
{
    ...
    [HideInInspector] public List<Creature> Targets = new List<Creature>();//the creatures that entered the sensor zone
    ...
    [HideInInspector] public Creature Target;//target to shoot at 
    private RaycastHit raycast;//raycast to chick if we can hit that target and it's not hiden behind another object
    private string[] layersToIgnor = new string[2];//layers to prevent the raycast from colliding with
    ...
    void FixedUpdate()
    {
        if (!IsItAirPlaneSensor)
        {
            ...
            {
                if (!SecurityWeapon.IsShooting && (Target == null || Target.WasDied))//if the waepon is not busy and the target is not null or dead
                {
                    if (Targets.Count > 0)//if there are Targets inside the security area
                    {
                        foreach (Creature tar in Targets)//find a one target from the Targets that had entred the security area
                        {
                            if (tar != null && !tar.WasDied)//if this creature is not null and not dead
                            {
                ...
                {
                    if (!CheckIfClearToShootAt(Target) || Target.WasDied)
                    {
        ...
        else
        {
            ...
            {
                if (!FighterAirPlane.IsShooting && (Target == null || Target.WasDied))//if the waepon is not busy and the target is not null or dead
                {
                    if (Targets.Count > 0)//if there are Targets inside the security area
                    {
                        foreach (Creature tar in Targets)//find a one target from the Targets that had entred the security area
    ...
    private bool CheckIfClearToShootAt(Creature target)//shoot a ray towards the target to know if we can shoot him and not hiden behind another object(a rock or something)
    {
        if (target != null)
        {
    ...
    void OnDrawGizmos()//drow a ray which the sensor use it(helps to visualies things)
    {
        if (!IsItAirPlaneSensor)
        {
            if (SecurityWeapon.HasToDefend && Target != null && !Target.WasDied)
            {
        ...
        {
            if (FighterAirPlane.HasToDefend && Target != null && !Target.WasDied)
            {
                Gizmos.color = new Color(1, 0, 0, 1);
                Gizmos.DrawRay(transform.position, transform.forward * (Vector3.Distance(transform.position, Target.RigBody.transform.position)));
            }
    ...
    public void RemoveDeadTarget(Creature creature)//remove the dead creature from Targets list after it dies
    {
        if (creature != null)
        {

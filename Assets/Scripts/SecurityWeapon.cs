...
public class SecurityWeapon : MonoBehaviour
{
    ...
    [SerializeField] private List<Projectile> airRockets = new List<Projectile>();
    ...
    void Update()
    {
        ...
        if (weaponType == Constants.SecurityWeaponsTypes.ground && currentBulletsNumber > 0)
        {
            if (!IsShooting)
            {
                rotatingPart.Rotate(Vector3.right * 200 * Time.deltaTime);
            }
            else
            {
                rotatingPart.Rotate(Vector3.right * 500 * Time.deltaTime);
            }
        }

        if (isEscorting)
        {
            LookTowardsEscortPoint(escortPoint);
        }
        else if (hasToResetRotation)
        {
            ResetRotation();
        }
    }
    ...
    {
        if (hasToLookAtTheTarget)
        {
            LookAtTheTarget(target);
        }
    }

    private void ShowUpdateImages()
    {
        if (weaponHealth != initialHealth || currentBulletsNumber != maxBulletsNumber)
        {
    ...
    private void LookTowardsEscortPoint(Vector3 escortPoint)//to let the weapon look towards random points(escorting area)
    {
        ...
        if (Quaternion.Angle(transform.rotation, targetAngle) > .2f)
        {
            transform.rotation = Quaternion.Slerp(transform.rotation, targetAngle, smoothWeaponEscortingSpeed * Time.deltaTime);
        }
        else
        {
            GenerateEscortPoint();
    ...
    private IEnumerator Shoot(Transform target)//to creat projectiles and shoot them towards the target
    {
        while (IsShooting)
        {
            if (target == null || currentBulletsNumber <= 0)
            {
                break;
            }
                ...
                foreach (Projectile rocket in airRockets)
                {
                    if (rocket != null && !rocket.IsUsed && !isCoolingDown)
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            rocket.FollowTarget(target.GetComponent<Creature>());
                        }
            ...
            else if (weaponType == Constants.SecurityWeaponsTypes.ground)
            {
                if (currentBulletsNumber > 0 && !isCoolingDown)
                {
                    ...
                    projectileScript.FollowTarget(target.GetComponent<Creature>());
                    StartCoroutine(SetCoolDownTime());
                }
            }
            yield return null;
    ...
    {
        if (Quaternion.Angle(transform.rotation, originalEuralAngles) >= .2f)
        {
            transform.rotation = Quaternion.Slerp(transform.rotation, originalEuralAngles, smoothWeaponEscortingSpeed * Time.deltaTime);
        }
        else
        {
            hasToResetRotation = false;
    ...
    {
        ammoStateText.text = currentBulletsNumber.ToString() + "/" + maxBulletsNumber.ToString();
    }

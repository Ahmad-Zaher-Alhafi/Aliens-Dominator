...
{
    [SerializeField] private Vector3[] points = new Vector3[] { Vector3.zero, Vector3.up };
    ...
    private enum Mode { PingPong, Loop }
    ...
    private void UpdateFacing()
    {
        if (faceTarget)
        {
            transform.LookAt(worldPoints[targetIndex]);
        }
    }
    ...
    {
        if (pingPongStep > 0 && targetIndex == points.Length - 1)
        {
            pingPongStep = -1;
        }
        else if (pingPongStep < 0 && targetIndex == 0)
        {
            pingPongStep = 1;
        }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        for (int i = 0; i < points.Length; i++)
        {
            ...
            if (i != 0 || movementMode != Mode.PingPong)
            {

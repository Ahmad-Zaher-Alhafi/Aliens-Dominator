...
namespace Defence_Weapons {
    public class SecuritySensor : MonoBehaviour {
        ...
        [HideInInspector] public List<Creature.Creature> Targets = new(); //the creatures that entered the sensor zone
        ...
        [HideInInspector] public Creature.Creature Target; //target to shoot at 
                ...
                if (SecurityWeapon.HasToDefend) {
                    if (!SecurityWeapon.IsShooting && (Target == null || Target.WasDied)) //if the waepon is not busy and the target is not null or dead
                        if (Targets.Count > 0) //if there are Targets inside the security area
                            foreach (Creature.Creature tar in Targets) //find a one target from the Targets that had entred the security area
        ...
        private bool CheckIfClearToShootAt(Creature.Creature target) //shoot a ray towards the target to know if we can shoot him and not hiden behind another object(a rock or something)
        {
            if (target != null) {
        ...
        public void RemoveDeadTarget(Creature.Creature creature) //remove the dead creature from Targets list after it dies
        {
            if (creature != null)

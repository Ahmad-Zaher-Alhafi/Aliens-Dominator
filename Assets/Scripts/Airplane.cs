...
{
    private bool isItBusy = false;//at normal situation one airplane is gonna delever the drops but in case that the player took two of the supplies caller at the same time then we have to create and send another temporary airplane if the main one is still busy
    ...
    public IEnumerator DropSupplies(Constants.SuppliesTypes suppliesType)
    {
        ...
        switch (suppliesType)
        {
            case Constants.SuppliesTypes.ArrowUpgrade:
                supplies = Instantiate(suppliesPrefab, dropCreatingPoint.position, Quaternion.identity);break;//create the drop
            case Constants.SuppliesTypes.RocketsAmmo:
                supplies = Instantiate(rocketsAmmoPrefab, dropCreatingPoint.position, Quaternion.identity); break;//create the drop
            case Constants.SuppliesTypes.BulletsAmmo:
                supplies = Instantiate(bulletsAmmoPrefab, dropCreatingPoint.position, Quaternion.identity); break;//create the drop

            default: break;
        }
        
        Rigidbody supRig = supplies.GetComponent<Rigidbody>();
    ...
    /// <summary>
    /// get the airplane back to it's initial posiotn after it disappears from the player view field to make it ready to drop another one later
    /// </summary>
    /// <returns></returns>

...
namespace Panteleymonov
{
  ...
  [ExecuteInEditMode]
  [AddComponentMenu("LowPolyGenerator/SimpleGenerator")]
  public class LowSimpleGenerator : MonoBehaviour
  {
    public enum SSSType
    {
      Box = 0,
    ...
    public Mesh NewGen()
    {
      ...
      {
        Vector3 pos = Vector3.Scale(LowPolyGenerator.VecrotNoize(i + SpraySeed) * 2.0f, PlaneScale);
    ...
    void GenIcosahedron(Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      {
        v[i] += LowPolyGenerator.VertexNoize(v[i] + From, seed_tmp, NoiseForce);
      ...
      if (SurfaceType == SSSType.Noise)
      {
        for (int j = 0; j < 10; j++)
          ...
          {
            SphereScaleSurfaceMod(ref v, LowPolyGenerator.VecrotNoize(seed_tmp + i).normalized * 0.9f, 1.0f, SSSModCurve, SSSModRadius);
          }
        for (int i = 0; i < vcount; i++)
        {
          v[i] = Quaternion.FromToRotation(LowPolyGenerator.VecrotNoize(seed_tmp + SSSModCount - 1).normalized, Vector3.down) * v[i];
        }
      }
    ...
    void GenCrystal(Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      {
        Normals[i] = Vector3.Normalize(Normal + LowPolyGenerator.VertexNoize(Poss[i], NoiseSeed, 1.0f) * DownForse);
      ...
      LowPolyGenerator.Tube(ref rock_tmp, SSSModCount, Poss, Scales, Normals, Tangents);
      for (int i = l; i < rock_tmp.vec.Length; i++)
      {
        rock_tmp.vec[i] += LowPolyGenerator.VertexNoize(rock_tmp.vec[i], NoiseSeed, NoiseForce);
      }
    }
    ...
    void GenMushroom(Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      {
        Vector3 noise = LowPolyGenerator.VertexNoize(Normal * i + From, NoiseSeed, NoiseForce);
      ...
      LowPolyGenerator.Tube(ref rock_tmp, SSSModCount, Poss, Scales, Normals, Tangents, 0, 3 + (Detail) * 2);
      LowPolyGenerator.Tube(ref sub_tmp, SSSModCount, Poss, Scales, Normals, Tangents, 3 + (Detail) * 2, (Detail + 3));
    }
    ...
    void Grass(Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      for (int i = 0; i < Detail + 1; i++)
      {
        Normals[i] = Vector3.Normalize(Normal + LowPolyGenerator.VertexNoize(Poss[i], NoiseSeed, 1.0f) * DownForse + NF * (i * i * std * std));
      ...
      Poss[Detail + 2] = Poss[Detail + 1] + Vector3.Normalize(Normal + LowPolyGenerator.VertexNoize(Poss[Detail+1], NoiseSeed, 1.0f) * DownForse + NF) * std * Scale.x;

      int l = rock_tmp.vec == null ? 0 : rock_tmp.vec.Length;
      LowPolyGenerator.Tube(ref rock_tmp, SSSModCount, Poss, Scales, Normals, Tangents,0,0x7FFFFFFF,45.0f);
      for (int i = l; i < rock_tmp.vec.Length; i++)
      {
        rock_tmp.vec[i] += LowPolyGenerator.VertexNoize(rock_tmp.vec[i], NoiseSeed, NoiseForce);
      }
    ...
    void Grows(Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      Poss[Detail + 2] = Poss[Detail + 1] + Vector3.Normalize(Normal + LowPolyGenerator.VertexNoize(Poss[Detail + 1], NoiseSeed, 1.0f) * DownForse + NF) * std * Scale.y;
    ...
    void CurvedWhirl(Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      for (int i = 0; i < Detail; i++)
      {
        ...
        Normals[i] = Vector3.Normalize(Vector3.up + LowPolyGenerator.VertexNoize(Poss[i], NoiseSeed, NoiseForce));

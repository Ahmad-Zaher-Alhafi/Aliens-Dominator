...
{
  [ExecuteInEditMode]
  [AddComponentMenu("LowPolyGenerator/TreeGenerator")]
  public class LowTreeGenerator : MonoBehaviour
  {
    ...
    public Mesh NewGen()
    {
      ...
      Segments = Mathf.Clamp(Segments, 1, 15);
    ...
    void GenTrunk(ref BranchProperties Branch, int Parts, Vector3 From, Vector3 Normal, Vector3[] RootRing)
    {
      ...
      for (int i = 0; i < Segs; i++)
      {
        ...
        br[i] = (Branch.iterator) < Branch.iteratorMax ? LowPolyGenerator.rnd(seed_tmp, maxb - minb) + minb : 0;
        seed_tmp++;
      ...
      for (int i = 0; i < Segs - 1; i++)
      {
        ...
        N = Vector3.Normalize(Normal + LowPolyGenerator.VertexNoize(Fromi, noiseseed_tmp, noiseforce_tmp) + NF);
    ...
    Vector3[] StartRing(int Parts, float Scale)
    {
      ...
      LowPolyGenerator.RingVectors(ref end, Parts, Vector3.one * Scale);
      return end;
    }
    ...
    Vector3[] GenTrunkSeg(int Iter, int Branches, int Parts, float Scale, float Twirl, Vector3 From, Vector3 To, Vector3 Normal, ref Vector3[] RootRing,bool show = true)
    {
      ...
      Matrix4x4 u = LowPolyGenerator.Matrix(Normal, RootRing[0].normalized);

      // generate end ring
      LowPolyGenerator.RingVectors(ref end,Parts, Vector3.one * Scale);
      LowPolyGenerator.TransformVectors(ref end, ref u);
      ...
      if (Branches > 0 && Parts > 1)
      {
        for (int i = 0; i < Parts; i++)
        {
          int idb = LowPolyGenerator.rnd(i + seed_tmp, Parts - 1);
          bool b = br[i];
          br[i] = br[idb];
          br[idb] = b;
        }
      ...
      // generate mesh
      LowPolyGenerator.TubeSegment(ref v, ref br, Parts, From, To, ref RootRing, ref end);
    ...
    void GenTrunkEnd(int Iter, int Branches, int Parts, float Twirl, Vector3 From, Vector3 To, Vector3 Normal, Vector3[] RootRing, bool show = true)
    {
      ...
      // generate mesh
      LowPolyGenerator.TubeEnd(ref v, ref br, Parts, From, To, ref RootRing);
    ...
    void GenMesh(int Detail, Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      Matrix4x4 u = LowPolyGenerator.Matrix(Normal, Tangent);
    ...
    void GenLeavesNormal(int Detail, Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      LowPolyGenerator.IcosahedronBegin(ref v);
      Detail = Mathf.Min(4, Detail);
      for (int i = 0; i < Detail; i++)
      {
        Vector3[] newv = new Vector3[v.Length * 4];
        LowPolyGenerator.IcosahedronDetailed(ref v, ref newv);
        v = newv;
      }
      vcount = v.Length;
      ...
      for (int i = 0; i < vcount; i++)
      {
        ...
        v[i] = Vector3.Scale((v[i] + LowPolyGenerator.VertexNoize(v[i], LeavesNoiseSeed, LeavesNoiseForce)), Scale) + From;
      }
    ...
    void GenLeavesNeedle(int Detail, Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent)
    {
      ...
      // generate cone
      LowPolyGenerator.RingVectors(ref begin, Detail, Vector3.one * Scale.x * 0.9f);
      LowPolyGenerator.TransformVectors(ref begin, ref u);
      LowPolyGenerator.RingVectors(ref end, Detail, Vector3.one * Scale.x);
      ...
      LowPolyGenerator.TubeSegment(ref v, ref bt, Detail, From, From + Normal * 0.1f * Scale.y, ref begin, ref end);
      for (int i = 0; i < v.Length; i++)
        v[i] += LowPolyGenerator.VertexNoize(v[i], LeavesNoiseSeed, LeavesNoiseForce);
      leaves_tmp.AddVertex(v, Detail * 4, 4);
      v = new Vector3[Detail * 3];
      LowPolyGenerator.TubeEnd(ref v, ref bt, Detail, From + Normal * 0.1f * Scale.y, From + Normal * Scale.y, ref end);
      for (int i = 0; i < v.Length; i++)
        v[i] += LowPolyGenerator.VertexNoize(v[i], LeavesNoiseSeed, LeavesNoiseForce);
      leaves_tmp.AddVertex(v, Detail * 3, 3);
      Array.Reverse(begin);
      LowPolyGenerator.TubeEnd(ref v, ref bt, Detail, From, From, ref begin);
      for (int i = 0; i < v.Length; i++)
        v[i] += LowPolyGenerator.VertexNoize(v[i], LeavesNoiseSeed, LeavesNoiseForce);
      leaves_tmp.AddVertex(v, Detail * 3, 3);
    }
    ...
    void GenLeavesPalm(int Detail, int Parts,float Rotate,Vector3 Scale, Vector3 From, Vector3 Normal, Vector3 Tangent, float Twirl)
    {
      ...
      LowPolyGenerator.RingVectors(ref normals, Parts, Vector3.one);
      for (int k = 0; k < Parts; k++)
        normals[k] = Vector3.Lerp(normals[k], Vector3.up, Rotate);
      LowPolyGenerator.TransformVectors(ref normals, ref u);
      bool[] bt = null;

      LowPolyGenerator.RingVectors(ref end, 4, Vector3.one);
      ...
      for (int k = 0; k < Parts; k++)
      {
        ...
        {
          Matrix4x4 b = LowPolyGenerator.Matrix(normals[k], Normal);
          ...
          LowPolyGenerator.TransformVectors(ref tmpb, ref b);
          Vector3[] v = new Vector3[4 * 3];
          Array.Reverse(tmpb);
          LowPolyGenerator.TubeEnd(ref v, ref bt, 4, Pos + Vector3.up * sinb * Scale.z, From, ref tmpb);
          for (int j = 0; j < v.Length; j++)
            v[j] += LowPolyGenerator.VertexNoize(v[j], LeavesNoiseSeed, LeavesNoiseForce);
          ...
          for (int i = 0; i < Detail; i++)
          {
            ...
            LowPolyGenerator.TransformVectors(ref tmpe, ref b);
            LowPolyGenerator.TubeSegment(ref v, ref bt, 4, Pos + Vector3.up * sinb * Scale.z, To + Vector3.up * sine * Scale.z, ref tmpb, ref tmpe);
            for (int j = 0; j < v.Length; j++)
              v[j] += LowPolyGenerator.VertexNoize(v[j], LeavesNoiseSeed, LeavesNoiseForce);
            tmpe.CopyTo(tmpb, 0);
            sinb = sine;
          ...
          v = new Vector3[4 * 3];
          LowPolyGenerator.TubeEnd(ref v, ref bt, 4, Pos + Vector3.up * sinb * Scale.z, Pos + normals[k] * Scale.y * 0.2f - Vector3.up * 0.5f, ref tmpb);
          for (int j = 0; j < v.Length; j++)
            v[j] += LowPolyGenerator.VertexNoize(v[j], LeavesNoiseSeed, LeavesNoiseForce);
          leaves_tmp.AddVertex(v, 4 * 3, 3);
        }

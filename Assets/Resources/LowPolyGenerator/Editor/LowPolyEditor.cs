...
namespace Panteleymonov
{
  public class MeshEditorWindow : EditorWindow
  {
    ...
    const string DotShaderName = "LowPolyGenerator/ViewDotMeshEditor";
    const string EdgeShaderName = "LowPolyGenerator/ViewEdgeMeshEditor";
    ...
    [MenuItem("Window/Low Poly Generator/Mesh Editor")]
    ...
    {
      MeshEditorWindow editor = (MeshEditorWindow)EditorWindow.GetWindow(typeof(MeshEditorWindow), false, "Mesh Editor");
      editor.Show();
      return editor;
    }
    ...
    {
      //dotMaterial = new Material(Shader.Find("LowPolyGenerator/ViewMeshEditor"));
    ...
    void RenderFace(Matrix4x4 matrix)
    {
      if (faceMaterial == null)
        faceMaterial = new Material(Shader.Find("LowPolyGenerator/ViewFaceMeshEditor"));
  ...
  [CustomEditor(typeof(LowSimpleGenerator))]
  public class LowSimpleGeneratorEditor : Editor
  {
    public override void OnInspectorGUI()
    {
      ...
      LowSimpleGenerator gen = (LowSimpleGenerator)target;
      if (GUILayout.Button(new GUIContent("Save Template", "Save to XML")))
      {
        string path = EditorUtility.SaveFilePanel(
        "Save template as XML",
        Application.dataPath + "/LowPolyGenerator/Temlpates",
        gen.name + ".xml",
        "xml");
        if (path.Length != 0)
    ...
    public void SaveXml(string file, ref LowSimpleGenerator gen)
    {
      ...
      XmlElement BaseGenerator = xml.CreateElement(string.Empty, "LowSimpleGenerator", string.Empty);
    ...
    public void LoadXml(string file, ref LowSimpleGenerator gen)
    {
      XmlDocument xml = new XmlDocument();
      xml.Load(file);
      ...
      if (body != null)
        ...
        {
          if (child.Name == "LowSimpleGenerator")
          {
            ...
            foreach (XmlNode PartsXML in child.ChildNodes)
            {
              ...
              {
                gen.SurfaceType = (LowSimpleGenerator.SSSType)Enum.Parse(typeof(LowSimpleGenerator.SSSType), PartsXML.Attributes.GetNamedItem("SurfaceType").Value);
  ...
  [CustomEditor(typeof(LowTreeGenerator))]
  public class LowTreeGeneratorEditor : Editor
  {
    public override void OnInspectorGUI()
    {
      base.OnInspectorGUI();
      GUILayout.BeginHorizontal();
      //Settings sett = (Settings)target;
      LowTreeGenerator gen = (LowTreeGenerator)target;
      if (GUILayout.Button(new GUIContent("Save Template", "Save to XML")))
      {
        string path = EditorUtility.SaveFilePanel(
        "Save template as XML",
        "/Assets/LowPolyGenerator/Temlpates",
        gen.name + ".xml",
        "xml");
        if (path.Length != 0)
          SaveXml(path, ref gen);
    ...
    public void SaveXml(string file, ref LowTreeGenerator gen)
    {
      XmlDocument xml = new XmlDocument();
      XmlDeclaration xmlDeclaration = xml.CreateXmlDeclaration("1.0", "UTF-8", null);
      XmlElement root = xml.DocumentElement;
      ...
      XmlElement BaseGenerator = xml.CreateElement(string.Empty, "LowTreeGenerator", string.Empty);
      BaseGenerator.SetAttribute("Version", "1.0");
      body.AppendChild(BaseGenerator);
    ...
    public void LoadXml(string file, ref LowTreeGenerator gen)
    {
      XmlDocument xml = new XmlDocument();
      xml.Load(file);
      ...
      if (body != null)
        foreach (XmlNode child in body.ChildNodes)
        {
          if (child.Name == "LowTreeGenerator")
          {
            //string version = child.Attributes.GetNamedItem("Version").Value;
            foreach (XmlNode PartsXML in child.ChildNodes)
            {
              ...
              {
                gen.LeavesType = (LowTreeGenerator.LeafType)Enum.Parse(typeof(LowTreeGenerator.LeafType),PartsXML.Attributes.GetNamedItem("Type").Value);
